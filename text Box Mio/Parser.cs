...
//    public static int nroInstrParaRectificarElWhile; 
	...
	static void Program() {
        Program1.form1.treeView1.ExpandAll();
        System.Windows.Forms.TreeNode Padre = new System.Windows.Forms.TreeNode();
        if (Parser.muestraProducciones)
        {
            Code.seleccLaProdEnLaGram(0);  //"Program = 'class' ident Declarations '{' MethodDeclsOpc '}'.");
            Code.cargaProgDeLaGram("Program = 'class' ident PosDeclars '{' MethodDeclsOpc '}'.");
       
            Padre.Text = "Program = 'class' ident PosDeclars '{' MethodDeclsOpc '}'.";
            Program1.form1.treeView1.Nodes.Add(Padre);
        }
        Check(Token.CLASS);   //class P
        Code.coloreaConRojo(true);   //colorea "class" en ventana de Edicion
        Padre.Nodes.Add("class");
        Check(Token.IDENT); // P => debo insertar el token en la tabla de símbolos
                            // es el comienzo del programa y abrir un nuevo alcance
        Code.coloreaConRojo(true);  //class "Prog"  en Edicion
        ...
        //Ya analizó Class P
        //Ahora viene
        // { ConstDecl | VarDecl | ClassDecl } => {"const"..|Type ident..|"class"...}
        // "{" { MethodDecl } "}".
        //pag 19 de Parsing.PPT

        //Declaraciones (de ctes, de Globals(aunque diga de vars) y de clases) 
        //hasta q venga una "{"
        System.Windows.Forms.TreeNode hijopromgra = new System.Windows.Forms.TreeNode("PosDeclars");
        Padre.Nodes.Add(hijopromgra);
        bool bandarita = false; // bandera se encarga de verificar si existen declaraciones globales,constantes etc.
        
        while (la != Token.LBRACE && la != Token.EOF) //zz3
        {
            Code.coloreaConRojo(false); //colorea "int" en int i;  yaPintado = true
            ...
            hijopromgra.Nodes.Add(hijodeclar); bandarita = true; 
            switch (la)
            {
                case Token.CONST:
        ...
        if (bandarita == false)
        {
            ...
            hijopromgra.Nodes.Add("PosDeclars = .");
        }
        if (ZZ.parser)
        {
        ...
        // la pertenece a First(MethodDec)  //ahora sólo deben haber metodos
        while ((la == Token.IDENT || la == Token.VOID) && la != Token.EOF)
        //////////////////////////////////////////////////////////////////////////////////////////////////
        {
            ///////////////////
            System.Windows.Forms.TreeNode hijo1 = new System.Windows.Forms.TreeNode("MethodDecl");
            Padre.Nodes.Add(hijo1);
            //////////////////
            MethodDecl(hijo1);  //void Main() int x,i; {val = new Table;....}
        }
        //////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 /*   static void ActParList()
    ...
    */
    ...
    //public static void Parse(TestTextWriter ss)
    //{ //Parse(Console.Out); 
    //}
